...
const Body = () => {
    ...
    const fetchData = async ()=>{
      ...
      setFilteredListOfRest(json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants);
    ...
    <div className="Body">
      <div className="filter">
        ...
        <button className="filter-btn" onClick={()=>{
            const filteredList = listOfRest.filter((rest)=>rest.info.avgRating>=4.5);
            setFilteredListOfRest(filteredList);
        }}>Top Rated</button>
      </div>
      <div className="restroCardContainer">
        {filteredListOfRest.map((restaurant)=><RestaurantCardComponent key={restaurant.info.id}  resData={restaurant} />)}
      </div>
    </div>
  );
};
